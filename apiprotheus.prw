...
/*/
//-------------------------------------------------------------------
WSMETHOD GET lucroVenda WSRECEIVE SearchKey, Status, Page, PageSize WSSERVICE TESTE
	    ...
	    If (cAlias)->(!EOF())
	        ...
	        While (cAlias)->(!EOF())
	            ...
	            If (nCount >= nStart) 
	                ...
	                aJsonProd[nEntJson]["code"				]	:= (cAlias)->cod 				
					aJsonProd[nEntJson]["description"		]	:= Alltrim((cAlias)->descr)				
					aJsonProd[nEntJson]["sale_price"		]	:= (cAlias)->sale_price
					aJsonProd[nEntJson]["standard_price"	]	:= (cAlias)->std_price
					aJsonProd[nEntJson]["profit"			]	:= (cAlias)->profit
					aJsonProd[nEntJson]["number"			]	:= (cAlias)->pedido
					If ((cAlias)->profit > 0)
						aJsonProd[nEntJson]["profit_status"		]	:= "success"
						aJsonProd[nEntJson]["profit_message"	]	:= "Lucro"
					Elseif((cAlias)->profit == 0)
						aJsonProd[nEntJson]["profit_status"		]	:= "warning"
						aJsonProd[nEntJson]["profit_message"	]	:= "Neutro"
					Else
						aJsonProd[nEntJson]["profit_status"		]	:= "danger"
						aJsonProd[nEntJson]["profit_message"	]	:= "Prejuizo"		
					EndIf
...
Static Function GetLucroVenda(cAliasQry)
